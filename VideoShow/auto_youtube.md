...
최적화 세부 사항
1.  Whisper 동적 모델 선택:
	•  기준: 오디오 길이(초) → tiny(<120초), base(120~300초), small(>300초).
	•  성능:
		•  tiny: ~39MB, CPU에서 5분 오디오 약 1~2분 처리.
		•  base: ~74MB, CPU에서 5분 오디오 약 2~3분.
		•  small: ~244MB, GPU 권장, 5분 오디오 약 30~40초(GPU T4).
	•  캐싱: cache/transcription_<hash>.srt로 전사 결과 저장, 재처리 시간 0초.
	•  세그먼트: 30초 단위 처리, 메모리 사용량 최대 1GB 미만(base 기준).
2.  M2M100 번역 최적화:
	•  분할 처리: 1000자 단위로 텍스트 분할, truncation=True로 안정성 확보.
	•  캐싱: cache/translation_<hash>.txt로 번역 결과 저장, 동일 텍스트 재번역 시 즉시 로드.
	•  성능: 1000자 번역 약 2~3초(GPU), 캐싱 시 0초.
	•  품질: Helsinki-NLP 대비 더 자연스러운 영어 출력, 특히 뉴스 문맥에서 강점.
3.  타임스탬프 동기화:
	•  Whisper의 세그먼트별 타임스탬프를 한국어/영어 자막에 동일 적용.
	•  영어 자막은 번역된 텍스트를 세그먼트 단위로 매핑, 동기화 유지.
4.  코드 구조:
	•  CONFIG 딕셔너리로 설정 중앙화.
	•  공통 캐싱 로직(save_to_cache, load_from_cache)으로 코드 재사용성 향상.
	•  함수별 단일 책임 원칙 준수, 주석으로 가독성 강화.
