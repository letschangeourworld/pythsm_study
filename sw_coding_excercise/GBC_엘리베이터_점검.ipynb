...
while i < N:                                   # 기준 구간수인 N 만큼 반복문 실행
    diff_dis = test[j][0]-stan[i][0]           # (j번째 시범구간길이)과 (i번째 기준구간길이)의 차이
    diff_v = test[j][1]-stan[i][1]             # (j번째 시범구간의 속도)과 (i번째 기준구간의 속도)의 차이
    
    if diff_v < 0:                             # 속도차가 음수일 때 (= 제한속도를 초과하지 않을 때)
        diff_v = 0                             # 속도차 변수에 0 입력
    
    if diff_dis >= 0:                          # 구간길이차가 0보다 같거나 클 때 (= 시범구간길이가 기준구간길이 보다 같거나 클 때)
        result.append([stan[i][0],diff_v])     # [i번째 기준구간길이, 해당하는 속도차]를 result 리스트에 입력 (현재까지 i = j)
        test[j][0] = diff_dis                  # 구간길이차를 j번째 시범구간길이에 덮어씌워 입력 (현재까지 i = j)
        i += 1                                 # 기준구간 인덱스 i에 1을 더해 줌 (여기는 i+1, j)
        if diff_dis == 0:                      # 구간길이차가 0이면,(= 구간길이차가 0이 되기 전까지는 j 숫자가 증가하지 않음)
            j += 1                             # 시범구간 인덱스 j에 1을 더해 줌 (여기는 다시 i+1, j+1)
    
    elif diff_dis < 0:                         # 구간길이차가 0보다 작을 때 (= 시범구간길이가 기준구간길이 보다 작을 때)
        result.append([test[j][0],diff_v])     # [j번째 시범구간길이, 해당하는 속도차]를 result 리스트에 입력 (현재까지 i = j)
        stan[i][0] = -diff_dis                 # 구간길이차를 양수로 만들어서 i번째 기준구간길이에 덮어씌워 입력 (현재까지 i = j)
        j += 1                                 # 시범구간 인덱스 j에 1을 더해 줌 (여기는 i, j+1)

max_v = result[0][1]                           # result 리스트의 0번째 열 요소의 1번째 요소 (= 임의의 속도차값 선택)

# 최대값 판정하기
for i in result:
    if i[1] > max_v:                           # result 속도차 값들 중에서 임의로 선택한 속도차값 보다 큰 값이 있으면, 
        max_v = i[1]                           # 그 값을 max_v로 다시 입력
