...
for r in matched_rst:
    for d in r:
        cv2.drawContours(new_canvas, d['contour'], -1, (255,255,255), 2)
        cv2.rectangle(new_canvas,
                      pt1 = (d['x'], d['y']),
                      pt2 = (d['x'] + d['w'], d['y'] + d['h']),
                      color = (255,255,255),
...
plate_imgs = []
plate_infos = []
for i, matched_chars in enumerate(matched_rst):
    ...
    img_rotated = cv2.warpAffine(new_canvas, M = rotation_matrix, dsize = (width, height))
...
for i, plate_img in enumerate(plate_imgs):
    img = cv2.resize(plate_img, dsize = (0,0),fx = 1.6, fy = 1.6)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    _, img = cv2.threshold(img, thresh, maxval, cv2.THRESH_BINARY | cv2.THRESH_OTSU)
    contours, _ = cv2.findContours(img, cv2.RETR_LIST, cv2.CHAIN_APPROX_NONE)
    
    plate_max_x, plate_max_y = img.shape[1], img.shape[0]
    ...
    for contour in contours:
        ...
        img_rst = img[plate_min_y:plate_max_y, plate_min_x:plate_max_x]
        img_rst = cv2.GaussianBlur(img_rst, ksize = (3, 3), sigmaX = 0)
        ret, img_rst = cv2.threshold(img_rst, thresh = 0, maxval = 255, type = cv2.THRESH_BINARY_INV)
        img_rst = cv2.copyMakeBorder(img_rst,
                                     top = 10,
                                     bottom = 10,
                                     left = 10,
                                     right = 10,
...
plt.figure(figsize=(12,10))
plt.title('img_size : {} X {}'.format(img_rst.shape[1], img_rst.shape[0]))
plt.imshow(img_rst, 'gray')
plt.show()

chars = pytesseract.image_to_string(img_rst, lang='kor', config='')
print('Number is :', chars)
~~~
